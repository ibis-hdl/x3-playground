...
struct bit_string_literal : x3::parser<bit_string_literal>
{
               ...
               [[maybe_unused]] RContextT const& rctx, attribute_type& attribute) const
    {
        ...
        attribute.value = convert<attribute_type::value_type>(
            attribute.literal, attribute.base);
...
private:
    ...
    std::optional<TargetT> convert(auto range, unsigned base) const
    {
        ...
        // prune the literal and copy result to string; std::from_chars() works with raw 
        // char pointer hence we use a string_view
        std::string const pruned = ranges::to<std::string>(
            range | views::filter([](char chr) { return chr != '_'; }));

        std::string_view const sv(pruned);
        auto const end = sv.data() + sv.size();
        
        TargetT result;
        auto const [ptr, errc] = std::from_chars(sv.data(), end, result, base);
        ...
        auto const ec = [=]() {
            ...
            if(errc != std::errc{}) {
                // std::errc::result_out_of_range, std::errc::invalid_argument ...
